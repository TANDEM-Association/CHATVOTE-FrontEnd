...
import { type Thesis } from "@/lib/chatvote-swiper/chatvote-swiper-store.types";
...
function ThesisCard({
  ...
  const manuallyRemoveCard = async (direction: SwipeType) => {
  ...
  const BackButton = (
    <Button
      variant="ghost"
      size="icon"
      className="text-muted-foreground absolute top-2 left-2 size-8 text-xs"
      onClick={handleBack}
      disabled={!active}
    >
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          commonClassNames,
          "inset-1 h-auto w-auto rounded-[13px] p-4 shadow-none",
        )}
        tabIndex={-1}
      >
        {content}
