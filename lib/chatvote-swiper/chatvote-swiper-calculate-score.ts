...
import {
  type ChatvoteSwiperResultHistory,
  type PartiesScoreResult,
} from "./chatvote-swiper.types";

export async function chatvoteSwiperCalculateScore(
  userResponses: ChatvoteSwiperResultHistory,
) {
  const theses = await getChatvoteSwiperTheses();
