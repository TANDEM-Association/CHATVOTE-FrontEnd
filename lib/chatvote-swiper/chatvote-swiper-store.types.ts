...
import { type SwipeType } from "./chatvote-swiper.types";
...
export type ChatvoteSwiperStoreState = {
  allTheses: Thesis[];
  thesesStack: Thesis[];
...
export type ChatvoteSwiperStoreActions = {
...
export type ChatvoteSwiperStore = ChatvoteSwiperStoreState &
  ChatvoteSwiperStoreActions;

export type ChatvoteSwiperStoreActionHandlerFor<
  T extends keyof ChatvoteSwiperStoreActions,
> = (
  get: () => ChatvoteSwiperStore,
  set: (
    nextStateOrUpdater:
      | ChatvoteSwiperStore
      | Partial<ChatvoteSwiperStore>
      | ((state: WritableDraft<ChatvoteSwiperStore>) => void),
    shouldReplace?: false,
  ) => void,
) => (
  ...args: Parameters<ChatvoteSwiperStoreActions[T]>
) => ReturnType<ChatvoteSwiperStoreActions[T]>;
