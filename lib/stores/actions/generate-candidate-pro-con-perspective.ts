...
export const generateCandidateProConPerspective: ChatStoreActionHandlerFor<
...
> =
  ...
  async (candidateId: string, message: MessageItem | StreamingMessage) => {
    ...
    if (!lastUserMessageBeforeProConPerspective || !message.content) {
      return;
    }

    socket.io?.generateCandidateProConPerspective({
