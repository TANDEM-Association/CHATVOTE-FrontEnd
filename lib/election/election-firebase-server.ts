...
async function loadMunicipalities(): Promise<Municipality[]> {
  ...
  try {
    ...
    const municipalitiesRef = collection(serverDb, "municipalities");
...
export async function getMunicipalityByCode(
  ...
  try {
    const serverDb = await getServerFirestore({ useHeaders: false });
    const docRef = doc(serverDb, "municipalities", code);
    ...
    return snapshot.data() as Municipality;
  } catch (error) {
...
async function getCandidatesImpl(): Promise<Candidate[]> {
  try {
    const serverDb = await getServerFirestore({ useHeaders: false });
    const candidatesRef = collection(serverDb, "candidates");
